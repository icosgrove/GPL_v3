...
m=0;
 k=1; 
  ...
  PARAMS.ltsa.indir=dir; % write current directory to PARAMS struct
  PARAMS.ltsa.fname(k,:)=fn; % write current filename to the first row, this looks like it might loop for multiple files
...
PARAMS.ltsahd.fname = char(zeros(PARAMS.ltsa.nxwav,40));         % make empty matrix - filenames need to be 40 char or less
        ...
        fid = fopen(fullfile(PARAMS.ltsa.indir,PARAMS.ltsa.fname(k,:)),'r'); % open current file to read
                
        fseek(fid,22,'bof'); % move poition to 22 bytes after begining of file
        PARAMS.ltsa.nch = fread(fid,1,'uint16');         % Number of Channels
        
        fseek(fid,34,'bof'); % move position to 34 bytes after beginning of file
        PARAMS.ltsa.nBits = fread(fid,1,'uint16');       % # of Bits per Sample : 8bit = 8, 16bit = 16, etc
        if PARAMS.ltsa.nBits == 16
            PARAMS.ltsa.dbtype = 'int16'; % add data type to PARAMS
        ...
        fseek(fid,80,'bof'); % move position to 80 bytes after beginning of file
        nrf = fread(fid,1,'uint16');         % Number of RawFiles in XWAV file (80 bytes from bof)

        fseek(fid,100,'bof'); % move position to 100 bytes after beginning of file
        ...
        for r = 1:nrf                           % loop over the number of raw files in this xwav file
            ...
            % [21 8 16 6 3 2.5] Which is YY:MM:DD HH:MM::SS of the
            % beginning of the current xwav. That is conververted to julian
